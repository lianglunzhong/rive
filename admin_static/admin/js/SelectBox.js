(function() {
    ...
    var SelectBox = {
        ...
        init: function(id) {
           ...
           $.ajax({     
                type:"POST",
                url:"/products/render_options/",
                dataType:"json",
                data:"",
                success:function(icon)
                {
                    var opt_num = box.options
                    var opt_len = box.options.length
                    for(var i=0; i < opt_len; i++)
                    {
                        var c1 = $.inArray(opt_num[i].value, icon["1"]);
                        var c2 = $.inArray(opt_num[i].value, icon["2"]);
                        if(c1>=0)
                        {
                            $(opt_num[i]).addClass("mv1");
                            //opt_num[i].addClass("mv1");          
                        }
                        else if(c2>=0)
                        {
                            $(opt_num[i]).addClass("mv2");      
                        }
                        else
                        {
                            $(opt_num[i]).addClass("mv");
                        } 
                    }
                }
            })
